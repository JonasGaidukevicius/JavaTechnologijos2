...
public class CartService {
	...
	public CartData getCart(String username) {
		...
		// atiduodami duomenys
		CartData cartToController = new CartData(cartFromDB.getUsername(), cartFromDB.getProductQuantity());
	...
	public void addProductToCart(String user, long productId, String productTitle, int quantity) {
		List<ProductQuantityInCart> productAlreadyInCart = new ArrayList<ProductQuantityInCart>();
		...
		if (productRepository.findProductById(productId) != null) {
			...
			productAlreadyInCart = currentCart.getProductQuantity();
			if (productAlreadyInCart == null) {
				currentCart.addQuantityToCart(new ProductQuantityInCart(productToCart, quantity));
			} else {
				int position = -1;
				for (int i = 0; i < productAlreadyInCart.size(); i++) {
					if (productAlreadyInCart.get(i).getProductInCart().getId() == productId) {
						position = i;
					}
				}
				if (position == -1) {
