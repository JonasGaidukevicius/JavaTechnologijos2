...
public class ProductController {
	...
	//vieno produkto gavimas
		...
		@ApiOperation(value = "Get products", notes = "Returns existing products")
		public Product getProductById(@ApiParam(value = "Product id", required = true) @Valid @PathVariable final long id) {
	...
	public void updateProduct(@ApiParam(value = "Product id", required = true) @Valid @PathVariable final long id,
		...
		productService.updateProduct(new Product(id, cmd.getTitle(), new ProductDetails(cmd.getImage(), cmd.getDescription()),
				cmd.getPrice(), cmd.getQuantity()));

	}
	
	//Kitoks kūrimo variantas, kai į Servisą paduodu ne gatavą objektą, o atskirus jo elementus.
	@RequestMapping(path="/another", method = RequestMethod.POST)
	@ResponseStatus(HttpStatus.CREATED)
	@ApiOperation(value = "Create new product", notes = "Creates new product with provided data")
	public void createAntotherProduct(
			@ApiParam(value = "Product data", required = true) @Valid @RequestBody final CreateProductCommand cmd) {

		productService.createAnotherProduct(cmd.getTitle(), cmd.getPrice(), cmd.getQuantity(), cmd.getImage(), cmd.getDescription());
		System.out.println("--------------------iš produkto kūrimo" + cmd);
	}
